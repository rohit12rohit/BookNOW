...
export const dismissReviewReportsAdminApi = async (reviewId) => {
    try {
        const response = await axios.put(`${API_URL}/reviews/${reviewId}/dismiss`);
        return response.data;
    } catch (error) {
        console.error(`Error dismissing reports for review ${reviewId} (Admin):`, error.response?.data || error.message);
        throw error.response?.data || new Error('Failed to dismiss review reports');
    }
};