...
/*
**
 * Fetches reported reviews for movies at the logged-in organizer's venues.
 * @returns {Promise<Array>} - Array of reported review objects.
 */
export const getReportedReviewsOrganizerApi = async () => {
    try {
        const response = await axios.get(`${API_URL}/reviews/reported`);
        return response.data;
    } catch (error) {
        console.error('Error fetching organizer reported reviews:', error.response?.data || error.message);
