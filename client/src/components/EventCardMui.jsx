// client/src/components/EventCardMui.jsx (Corrected)
...
// Accept event and isLoading prop
  ...
  if (isLoading) {
     return (
       <Card sx={{ maxWidth: 345, width: '100%', height: '100%' }}>
         <Skeleton variant="rectangular" sx={{ height: 140 }} /> {/* Match image height */}
         <CardContent>
   ...
   // If not loading but no event data
  ...
  return (
    ...
    >
      <CardActionArea component={RouterLink} to={`/events/${event._id}`} sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
        <CardMedia
          component="img"
          sx={{ height: 140, objectFit: 'cover' }} // Use sx prop
          image={imageUrl}
          alt={`${title} poster`}
