...
const HomePage = () => {
    ...
    const HorizontalScrollSection = ({ title, items, CardComponent, isLoading, error }) => {
                ...
                <Box sx={{ position: 'relative', mt: 2 }}>
                    ...
                    >
                        {(isLoading ? Array.from(new Array(6)) : items).map((item, index) => (
                             <Box key={isLoading ? `skeleton-${index}` : item._id} sx={{ flex: '0 0 auto', width: {xs: '180px', sm: '220px'}, scrollSnapAlign: 'start', height: '430px' }}>
                                <CardComponent movie={item} isLoading={isLoading} />
                            </Box>
                        ))}
                    </Box>
    ...
    const GridSection = ({ title, items, CardComponent, isLoading, error }) => (
        <Box component="section" sx={{ mb: 6 }}>
            <Typography variant="h4" component="h2" gutterBottom sx={{ fontWeight: 'bold', borderLeft: '4px solid', borderColor: 'error.main', pl: 1.5 }}>
                {title}
            </Typography>
            ...
            {isLoading ? (
                <Grid container spacing={3}>
                    {Array.from(new Array(4)).map((_, index) => (
                        <Grid item xs={12} sm={6} md={4} lg={3} key={`event-skeleton-${index}`}>
                ...
                <Grid container spacing={3}>
                    {items.map((item) => (
                        <Grid item xs={12} sm={6} md={4} lg={3} key={item._id}>
                            <CardComponent event={item} isLoading={false} />
                        </Grid>
                    ))}
                </Grid>
            ) : (
                !error && <Typography sx={{ textAlign: 'center', color: 'text.secondary', mt: 3 }}>No {title.toLowerCase()} found.</Typography>
            )}
        </Box>
    );
    ...
    return (
        <Container maxWidth="lg" sx={{ py: 4 }}>
            <HorizontalScrollSection title="Now Showing" items={nowShowingMovies} CardComponent={MovieCardMui} isLoading={loadingMovies} error={errorMovies} />
            <GridSection title="Upcoming Events" items={upcomingEvents} CardComponent={EventCardMui} isLoading={loadingEvents} error={errorEvents} />
        </Container>
    );
};
