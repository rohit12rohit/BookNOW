            ...
            <Paper elevation={2} sx={{ p: { xs: 2, md: 4 } }}>
                <Grid container spacing={4}>
                    <Grid item xs={12} md={4}>
                        <Box
                            component="img"
                            sx={{ width: '100%', height: 'auto', maxHeight: { xs: 400, md: 550 }, objectFit: 'contain', borderRadius: 2, boxShadow: 3, bgcolor: 'grey.200' }}
                            ...
                            return (
                                <Chip
                                    ...
                                    sx={{
                                        ...
                                        '&:hover': {
                                            bgcolor: isSelected ? 'error.dark' : 'grey.300'
                                        }
                                    }}
                                />
                            );
                                    ...
                                    <React.Fragment key={review._id}>
                                        <ListItem alignItems="flex-start" sx={{ flexDirection: 'column', py: 2 }}>
                                            ...
                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, pl: '56px', width: '100%' }}>
                                                <Button size="small" startIcon={<ThumbUpIcon />} onClick={() => handleInteraction(review._id, 'like')} color={review.likes.includes(user?._id) ? "primary" : "inherit"}>
                                                    {review.likes.length}
                                                </Button>
                                                <Button size="small" startIcon={<ThumbDownIcon />} onClick={() => handleInteraction(review._id, 'dislike')} color={review.dislikes.includes(user?._id) ? "error" : "inherit"}>
                                                    {review.dislikes.length}
                                                </Button>
                                                <Tooltip title="Report this review as inappropriate">
                                                    <IconButton size="small" onClick={() => {
                                                        if (!isAuthenticated) navigate('/login', { state: { from: { pathname: `/movies/${movieId}` } } });
                                                        else setReportState({ open: true, reviewId: review._id, reason: '' });
                                                    }}>
