...
exports.getBookingByIdAdmin = async (req, res) => {
    ...
    try {
        let booking = await Booking.findById(bookingId)
            ...
            .populate({
                path: 'showtime',
                // select: 'field1 field2 movie event venue ...' // Select all fields you need from Showtime
                populate: [ // Nested populate for references within showtime
                    // { path: 'movie', model: 'Movie', select: 'title duration genre movieLanguage posterUrl censorRating' },
                    // { path: 'event', model: 'Event', select: 'title category startDate endDate imageUrl' },
                    // The above with explicit model names SHOULD work. If it doesn't, Mongoose is deeply confused.
                    // Let's try populating them individually after the main showtime populate as a fallback for this specific case.
                    {
                        path: 'venue',
                        model: 'Venue', // Explicit model
                        select: 'name address facilities organizer',
                        populate: { path: 'organizer', model: 'User', select: 'name organizationName email' } // Explicit model
                    }
                ]
            })
            .populate({ path: 'promoCodeApplied', model: 'PromoCode' })
            .populate({ path: 'checkedInBy', model: 'User', select: 'name email' })
            .lean(); // Use lean for consistency if using in getAllBookings

        if (!booking) {
            return res.status(404).json({ msg: 'Booking not found' });
        }

        // Manual populate for showtime.movie and showtime.event if .lean() was used
        // and they weren't deeply populated correctly.
        if (booking.showtime) {
            if (booking.showtime.movie && typeof booking.showtime.movie === 'object' && !booking.showtime.movie.title) { // if movie is an ObjectId string or object without title
                const movieDoc = await Movie.findById(booking.showtime.movie._id || booking.showtime.movie)
                                            .select('title duration genre movieLanguage posterUrl censorRating')
                                            .lean();
                if (movieDoc) booking.showtime.movie = movieDoc;
            }
            if (booking.showtime.event && typeof booking.showtime.event === 'object' && !booking.showtime.event.title) { // if event is an ObjectId string or object without title
                const eventDoc = await Event.findById(booking.showtime.event._id || booking.showtime.event)
                                            .select('title category startDate endDate imageUrl')
                                            .lean();
                if (eventDoc) booking.showtime.event = eventDoc;
            }
        }
        ...
        res.status(500).json({ msg: 'Server error fetching booking details', error: err.message, path: err.path });
    }
};
...
// @desc    Get all reviews that have been reported
// @route   GET /api/admin/reviews/reported
// @access  Private (Admin Only)
exports.getReportedReviews = async (req, res) => {
    try {
        const reportedReviews = await Review.find({ 'reports.0': { $exists: true } }) // Find reviews where the reports array is not empty
            .populate('user', 'name email')
            .populate('movie', 'title')
            .populate('reports.user', 'name email') // Populate the user who reported
...
// @desc    Dismiss all reports for a specific review
// @route   PUT /api/admin/reviews/:id/dismiss
// @access  Private (Admin Only)
exports.dismissReviewReports = async (req, res) => {
    try {
        ...
        review.reports = []; // Clear the reports array
        await review.save();

        res.status(200).json({ success: true, msg: 'Reports for review have been dismissed.', review });
    } catch (err) {
        console.error('Error dismissing review reports (Admin):', err.message);
        res.status(500).json({ msg: 'Server error' });
    }
};