...
// @desc    Get reported reviews for movies shown at the organizer's venues
// @route   GET /api/organizer/reviews/reported
// @access  Private (Approved Organizer Only)
exports.getMyReportedReviews = async (req, res) => {
    try {
        const organizerId = req.user.id;

        // Find all venues managed by the organizer
        const managedVenueIds = await Venue.find({ organizer: organizerId }).distinct('_id');
        if (managedVenueIds.length === 0) {
            return res.status(200).json([]); // No venues, so no reviews
        }

        // Find all unique movie IDs from showtimes at those venues
        const relevantMovieIds = await Showtime.find({ venue: { $in: managedVenueIds } }).distinct('movie');
        if (relevantMovieIds.length === 0) {
            return res.status(200).json([]); // No movies shown, so no reviews
        }

        // Find all reviews for those movies that have reports
        const reportedReviews = await Review.find({
            movie: { $in: relevantMovieIds },
        ...
        console.error('Error fetching organizer reported reviews:', err.message);
        res.status(500).json({ msg: 'Server error' });
    }
};